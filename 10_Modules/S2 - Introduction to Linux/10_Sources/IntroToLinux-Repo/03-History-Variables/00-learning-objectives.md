# Learning Objectives

- Understand shell environment variables
- Learn about command history and shortcuts
- Master shell navigation features
- Understand variable creation and usage
- Work with the PATH environment variable
---

## Navigation

**Next:** [→ Review Shell Basics Lesson 12 Recap](01-review-shell-basics-lesson-12-recap.md)  
**Previous:** [← Next Lesson](../02-Shell/13-next-lesson.md)  
**Lesson Home:** [↑ Lesson 3: History & Variables](../)
**Course Home:** [⌂ Introduction to Linux](10_Modules/S2%20-%20Introduction%20to%20Linux/10_Sources/IntroToLinux-Repo/README.md)

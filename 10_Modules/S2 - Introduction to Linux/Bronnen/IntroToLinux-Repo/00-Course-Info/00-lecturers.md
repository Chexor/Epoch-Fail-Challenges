# Course Lecturers

## 👨‍🏫 Your Instructors

This Introduction to Linux course is taught by two experienced lecturers from VIVES University of Applied Sciences.

---

## 👤 Steven Moerman

<div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 20px; border-radius: 10px; color: white; margin: 20px 0;">

### 📧 Contact Information

- **Email:** steven.moerman@vives.be
- **Office:** HWB

### 🎓 Expertise & Teaching Areas

S. Moerman specializes in **Network and Security** domains:

- 🖥️ **Computer Systems**
- 🔒 **Network Security**
- 🤖 **Network Automation**
- 🌐 **Advanced Networking**

</div>

---

## 👤 Milan Dima

<div style="background: linear-gradient(135deg, #11998e 0%, #38ef7d 100%); padding: 20px; border-radius: 10px; color: white; margin: 20px 0;">

### 📧 Contact Information

- **Email:** milan.dima@vives.be
- **Office:** BHWB

### 🎓 Expertise & Teaching Areas

M. Dima brings expertise across multiple domains:

- 🔐 **Cybersecurity Architecture & Vulnerability Analysis**
- 🐧 **Introduction to Linux**
- 🚀 **DevOps Practices**
- ☁️ **Cloud Infrastructure**
- 🟢 **Node.js Backend Programming**
- 📱 **Swift UI iOS App Development**

</div>

---

## 📞 How to Reach Us

### For All Course-Related Questions:

- 💬 Use the course discussion forum
- 🏢 Visit during office hours

### For Personal Matters:

- 📧 Email us directly

### Best Practices for Communication:

1. ✉️ Use your **VIVES email** address
2. 📝 Include **clear subject lines** (e.g., "Linux - Question about permissions")
3. ⏰ Allow **24-48 hours** for email responses
4. 🔍 Check course materials and FAQ first

---

## 🎯 What to Expect

Throughout this course, you'll receive:

- 📚 **Comprehensive lectures** covering Linux fundamentals
- 💻 **Hands-on labs** for practical experience
- 🤝 **Personal support** from both instructors
- 📊 **Regular feedback** on your progress
- 🌟 **Real-world examples** from industry experience

---

## Navigation

**Next:** [→ Course Agreements](01-course-agreements.md)  
**Course Home:** [⌂ Introduction to Linux](10_Modules/S2%20-%20Introduction%20to%20Linux/Bronnen/IntroToLinux-Repo/README.md)

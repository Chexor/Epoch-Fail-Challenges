...
- `20_Concepts/` stores reusable concept notes that outlive a single course.
...
### `20_Concepts/` is the durable knowledge layer
...
- concept domains should default to `Topics/` plus `Maps/`
- larger domains may add `Comparisons/` or `Execution/` only when those note types provide recurring value
...
3. Distill reusable knowledge into `20_Concepts/` when it has cross-course value.
...
- keep `20_Concepts/` simple by default and expand its domain structure only when the extra layer clearly improves learning or retrieval
